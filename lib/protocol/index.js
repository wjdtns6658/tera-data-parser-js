...
class TeraProtocol {
  ...
  load(basePath = require.resolve('tera-data')) {
    ...
    for (let i = 0; i < mapData.length; i++) {
      ...
      if (!match) {
        log.warn(`[protocol] load - parse error: malformed line\n    at "${mapPath}", line ${i + 1}`);
        continue;
      }
      ...
      if (isNaN(code)) {
        log.warn(`[protocol] load - parse error: non-numeric opcode\n    at "${mapPath}", line ${i + 1}`);
        continue;
      }
    ...
    for (const file of files) {
      ...
      if (!parsedName) {
        if (file.endsWith('.def')) {
          log.warn(`[protocol] load - invalid filename syntax "${path.join(defPath, file)}"`);
        } else {
          log.debug(`[protocol] load - skipping non-.def path "${path.join(defPath, file)}"`);
        }
        continue;
      }
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (implicitMeta && (type === 'count' || type === 'offset')) {
          log.warn(`[protocol] load - parse warning: "count" or "offset" encountered, disabling implicit metatypes\n    at "${fullpath}", line ${i + 1}`);
          implicitMeta = false;
        }
        ...
        if (depth > level) {
          ...
          // sanity check
          if (depth !== level) {
            log.warn(`[protocol] load - parse warning: array nesting too deep\n    at "${fullpath}", line ${i + 1}`);
          }

...
class TeraProtocol {
  ...
  load(basePath = require.resolve('tera-data')) {
    ...
    function flatten(def) {
      const obj = def.meta.concat(
        def.map(([k, t]) => [k, Array.isArray(t) ? flatten(t) : t])
      );
      obj.type = def.type;
      return obj;
    }
    ...
    for (const file of files) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (type === 'count' || type === 'offset') {
          console.warn(`[protocol] load - parse warning: "count" and "offset" metatypes are deprecated, ignoring\n    at "${fullpath}", line ${i + 1}`);
          continue;
        }
      ...
      messages.get(name).set(version, flatten(definition));

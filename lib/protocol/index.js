...
class TeraProtocol {
    ...
    for (const [key, type] of definition) {
      if (Array.isArray(type)) {
        switch (type.type) {
          ...
          case 'object': {
            data[key] = this.parse(type, null, reader, `${displayName}.${key}`);
            break;
          }
  ...
  write(identifier, desiredVersion, data, writer, customName) {
    ...
    for (const [key, type] of definition) {
      ...
      if (Array.isArray(type)) {
        if (type.type === 'object') {
          this.write(type, version, value, writer, `${displayName}.${key}`);
          continue;
        }
        ...
        switch (type) {
          ...
          default: {
            ...
            try {
              ...
              err.message = [
                `[protocol] write - ${displayName}: error writing "${key}" (type: ${type})`,

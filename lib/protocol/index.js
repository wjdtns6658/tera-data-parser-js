...
class TeraProtocol {
   ...
   */
  resolveIdentifier(identifier, defaultName = '<Object>') {
   ...
   * @returns {Object}
   */
  parse(identifier, reader, customName) {
    const data = {};
    const { name, definition } = this.resolveIdentifier(identifier, customName);
    ...
    for (const [key, type] of definition) {
      ...
      if (Array.isArray(type)) {
        ...
        while (next) {
          ...
          next = reader.uint16();
          array[index++] = this.parse(type, reader, `${name}.${key}`);
   ...
   * @returns {Buffer}
   */
  write(identifier, data, writer, customName) {
    const { name, code, definition } = this.resolveIdentifier(identifier, customName);
      ...
      if (Array.isArray(type)) {
        ...
        if (length !== 0) {
          ...
          for (const element of value) {
            ...
            // recurse
            this.write(type, element, writer, `${name}.${key}`);
          }
        }
      // `type` is primitive
      } else {
